...
# --- 5. AFFICHAGE ADMIN REGROUPÉ (Onglets par utilisateur) ---
if code_saisi == MON_CODE:
    st.divider()
    st.header("📊 Historique des conversations")
    
    # On récupère tous les messages
    docs = db.collection("messages").order_by("date", direction=firestore.Query.ASCENDING).stream()
    
    # On regroupe tout par nom
    groupes = {}
    for doc in docs:
        d = doc.to_dict()
    ...
    if groupes:
        # Création des onglets : un seul onglet par nom !
        noms_des_onglets = list(groupes.keys())
        onglets = st.tabs(noms_des_onglets)
        
        for i, nom_onglet in enumerate(noms_des_onglets):
            with onglets[i]:
                st.subheader(f"Discussion avec {nom_onglet}")
                for m in groupes[nom_onglet]:
                    st.write(f"**Question :** {m.get('question')}")
                    st.write(f"**Réponse :** {m.get('reponse')}")
                    st.write("---")
    else:
        st.write("Aucun message dans la base.")
